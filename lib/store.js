...
import { SEED_CLIENTS } from './clients';
...
/** Seeds first, saved records on top — saved always wins on a slug collision. */
export async function listClients() {
  const saved = await readRaw();
  const bySlug = new Map(SEED_CLIENTS.map((c) => [c.slug, { ...c, seeded: true }]));
  for (const c of saved) bySlug.set(c.slug, { ...bySlug.get(c.slug), ...c, seeded: false });
...
export async function saveClient(client) {
  if (backend() === 'supabase') {
    ...
    const { seeded, ...clean } = { ...base, ...client };
